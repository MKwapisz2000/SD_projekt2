...
void Heap::insert(int e, int p)
{
	...
	//Stworzenie nowego węzła (elementu klejki)
	Node* new_node = new Node(e, p);

	//Dodanie węzła na koniec kopca (by zachować ciągłość kopca)
	if (size == 0)
	{
	...
	else
	{
		...
		//Indeks w tablicy elementu właśnie dodanego
		int i = size;

		//Indeks w tablicy rodzica właśnie dodanego elementu
		int j = (i - 1) / 2;

		//Zmieniamy ustawienie węzła dopóki jego priorytet jest większy od jego rodzica i nie jest korzeniem
...
void Heap::extract_max()
{
	...
	while (i < size)
	{
		//Sprawdzamy czy priorytet rodzica jes mniejszy niż priorytet dziecka lewego
		...
		{
			//Sprawdzamy czy priorytet rodzica jes mniejszy niż priorytet dziecka prawego
...
void Heap::increase_key(int e, int p, int i)
{
	...
	//Wykonujemy dopóki priorytet dziecka jest większy niż rodzica i jednocześnie dziecko nie jest korzeniem
